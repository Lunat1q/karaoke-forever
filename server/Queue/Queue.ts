...
class Queue {
   ...
   */
  static add ({ roomId, songId, userId, youtubeVideoId }: { roomId: number, songId?: number, userId: number, youtubeVideoId?: string }): void {
    const fields = new Map()
    fields.set('roomId', roomId)
    if (songId) fields.set('songId', songId)
    fields.set('userId', userId)
    if (youtubeVideoId) fields.set('youtubeVideoId', youtubeVideoId)
  ...
  static get (roomId: number): { result: number[], entities: Record<number, QueueItem> } {
    ...
    const query = sql`
      SELECT queueId, songId, queue.userId, prevQueueId, youtubeVideoId,
    ...
    const rows = db.all<{
      ...
      youtubeVideoArtist: string
      youtubeVideoTitle: string
      youtubeVideoDuration: number
      youtubeVideoKaraoke: number
    ...
    for (const row of rows) {
      ...
      if (row.youtubeVideoId) {
        ...
        if (row.youtubeVideoStatus === 'ready') {
          try {
            const tmpBase = prefs.tmpOutputPath || 'tmp'
            const karaokeFile = path.join(tmpBase, row.youtubeVideoId, 'karaoke.mp4')
            if (fs.existsSync(karaokeFile) && fs.statSync(karaokeFile).size >= 1000) {
              if (!row.youtubeVideoKaraoke) {
                const alignedFile = path.join(tmpBase, row.youtubeVideoId, 'aligned.txt')
                if (fs.existsSync(alignedFile) && fs.statSync(alignedFile).size >= 1000) {
                  entities[row.queueId].youtubeAlignedLyrics = JSON.parse(
                    fs.readFileSync(alignedFile, 'utf-8')
                  )
                }
              }
            }
        ...
        if (!pathData.has(row.pathId)) {
          pathData.set(row.pathId, row.pathData ? JSON.parse(row.pathData) : {})
        }
        ...
        entities[row.queueId].mediaType = 'unknown'
      }
