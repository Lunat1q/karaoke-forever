...
router.delete('/:roomId', (ctx) => {
  ...
  `
  const queueYoutubeItems = db.all<any>(String(queueYoutubeItemsQuery), queueYoutubeItemsQuery.parameters)
  ...
  // cleanup youtube videos that are no longer queued anywhere
  for (const video of queueYoutubeItems) {
